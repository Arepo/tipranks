...
class CompaniesFilter:
    ...
    def filter_by_market_cap(self, max_market_cap=5000000000):
        ...
        for company_name in self.companies:
            self._log_whether_smallcap(company_name, max_market_cap)

        self.counted_small_caps = Counter({
          company_name:count for (company_name, count) in
          self.companies.items() if self._tipranks_ticker(company_name) in
          self.small_caps})
    ...
    def _log_whether_smallcap(self, company_name, max_market_cap):
        tipranks_ticker = self._tipranks_ticker(company_name)
        market_cap = self._market_cap(company_name)
    ...
    def _market_cap(self, company_name):
        yf_ticker = self._yf_ticker(company_name)
    ...
    def _yf_ticker(self, company_name) -> str:
        return re.search('(\w+)', company_name).group(0)

    def _tipranks_ticker(self, company_name) -> str:
