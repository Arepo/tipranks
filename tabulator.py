...
class Tabulator:
    ...
    def write_to_analysts_companies(self,
                                    analyst,
                                    stock_evaluations,
                                    csv_title):
        headers = [
        ...
        with open(csv_title, 'a', newline='') as csvfile:
            ...
            row = { 'expertId': analyst['expertId'],
                    'expertSector': analyst['sectorId'] }
            for company_x_analyst in stock_evaluations:
                row = row | { key:company_x_analyst[key]
                              for key in headers_ex_expert }
                writer.writerow(row)

    def write_to_retail_analyst_portfolios(self,
                                           analyst,
                                           stock_evaluations,
                                           csv_title):
        with open(csv_title, 'a', newline='') as csvfile:
            ...
            row.extend([
                ...
                ) for company in stock_evaluations])
            writer.writerow(row)
